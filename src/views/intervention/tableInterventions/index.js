...
import { getAllIntervention, getInterventionsByReclamationId } from 'api/intervention';
...
const createData = (reclamation, idReclamation, handleEditClick, handleInterventionClick, handleDeleteClick) => {
    const { id, num_Contrat, adresseReclamation, priority, clientInfo, telportable, status, branch, branchid, adresse, comment } =
        reclamation;
    console.log();
    ...
    return {
        ...
        num_Reclamation: reclamation?.id ?? '-',
        equipe: reclamation?.equipe ?? '-',
        adresse: adresse ?? '-',
        description: comment ?? '-',
        createAt: date ?? '-',
        heure: time ?? '-',
...
const columns = [
    { id: 'id', label: 'N°intérvention', minWidth: 70 },
    { id: 'num_Reclamation', label: 'N°réclamation', minWidth: 70, align: 'left' },
    { id: 'description', label: 'Description', minWidth: 70, align: 'left' },
    { id: 'equipe', label: 'Equipe', minWidth: 90, align: 'left' },
    { id: 'createAt', label: 'Date', minWidth: 90, align: 'left' },
    { id: 'heure', label: 'Heure', minWidth: 90, align: 'left' },
    { id: 'adresse', label: 'Adresse', minWidth: 70, align: 'left' },
...
const TableIntervention = () => {
    ...
    const fetchPersonnelData = async () => {
        try {
            const { data: reclamationData } = await getAllIntervention(branchid);
