...
function ReclamationInfo() {
    ...
    useEffect(() => {
        if (branchid) {
            const fetchData = async () => {
                const { data: familleByBranch } = await getAllByBranch(getBranchById(branchid));
    ...
    const getType = async (idFamille) => {
        console.log('testerrrrrrrrr-> id :', idFamille);
        if (idFamille !== 0) {
            try {
                const { data: DataContrat } = await getAllByFamille(idFamille);
                setType(DataContrat);
    ...
    const handleInputChange = async (e) => {
        ...
        if (name === 'famille') {
            console.log('testerrrrrrrrr-> famille :', value);
            await getType(value);
